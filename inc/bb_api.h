...
#define BB_SET_PRJ_DISPATCH2        BB_REQUEST(BB_REQ_SET, 201)
...
typedef enum {
    ...
    BB_EVENT_PRJ_DISPATCH2,                                     /**<@note 项目自定义事件分发2*/
...
typedef struct {
    uint8_t         mode;                                       /**<@note BR跳频模式 */
    uint8_t         option;                                     /**<@note 预留扩展选项字段 */
...
typedef struct {
    ...
    int8_t          bonus_low_band;                             // TDB unit: db
...
/**定义设置命令BB_SET_POWER_SAVE_MODE_MODE的输入参数结构*/
typedef struct {
